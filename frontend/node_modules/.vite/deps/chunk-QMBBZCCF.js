...
function serializeTransactionEIP712(transaction) {
  const { chainId, gas, nonce: nonce2, to, from: from14, value, maxFeePerGas, maxPriorityFeePerGas, customSignature, factoryDeps, paymaster, paymasterInput, gasPerPubdata, data } = transaction;
...
function transactionToMessage(transaction) {
  const { gas, nonce: nonce2, to, from: from14, value, maxFeePerGas, maxPriorityFeePerGas, factoryDeps, paymaster, paymasterInput, gasPerPubdata, data } = transaction;
  return {
    txType: 113n,
    from: BigInt(from14),
    to: to ? BigInt(to) : 0n,
...
function serializeTransactionDeposit(transaction) {
  ...
  const { sourceHash, data, from: from14, gas, isSystemTx, mint: mint2, to, value } = transaction;
  const serializedTransaction = [
    sourceHash,
    from14,
    to ?? "0x",
...
function hash(authorization, options = {}) {
  ...
  return keccak2562(concat2("0x05", fromHex(toTuple2(presign ? {
...
function toRpc7(transaction, _options) {
  ...
  if (transaction.feePayerSignature) {
    ...
    rpc.feePayerSignature.v = fromNumber2(yParityToV((_a = transaction.feePayerSignature) == null ? void 0 : _a.yParity));
  }
  if (transaction.signature)
  ...
  if (!transaction.from && signatureEnvelope) {
    try {
      transaction.from = extractAddress({
        payload: getSignPayload3(from12(transaction)),
        signature: signatureEnvelope,
        root: true
      });
    } catch {
...
function serialize4(envelope, options = {}) {
    ...
    if (options.format === "feePayer" && signature) {
      ...
      if (sig.type === "secp256k1")
        return recoverAddress({
          payload: getSignPayload3(from12(envelope)),
  ...
  return concat2(options.format === "feePayer" ? feePayerMagic : serializedType, fromHex(serialized));
}
...
//# sourceMappingURL=chunk-QMBBZCCF.js.map
