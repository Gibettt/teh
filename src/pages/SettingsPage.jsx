        ...
        <div className="card p-6">
          ...
          <div className="mt-4 space-y-3 text-sm text-slate-600">
            <div className="surface-muted rounded-[24px] p-4">
              ...
              <div className="mt-1">{status?.storage?.enabled ? "Aktif" : "Belum aktif"}</div>
              <div className="mt-2 break-all text-xs text-slate-500">Project: {status?.storage?.projectRef || "-"}</div>
              <div className="mt-1 break-all text-xs text-slate-500">URL: {status?.storage?.url || "-"}</div>
              <div className="mt-1 text-xs text-slate-500">Key aktif: {status?.storage?.activeKeyType || "-"}</div>
              <div className="mt-1 text-xs text-slate-500">Publishable key: {status?.storage?.publishableKeyConfigured ? "Sudah diisi" : "Belum diisi"}</div>
            </div>
            <div className="surface-muted rounded-[24px] p-4">
        ...
        <div className="card p-6">
          ...
          <div className="mt-4 space-y-3 text-sm text-slate-600">
            <div className="surface-muted rounded-[24px] p-4">
              ...
              <div className="mt-2 text-xs text-slate-500">Isi VITE_WALLETCONNECT_PROJECT_ID di .env agar flow QR dan mobile wallet bisa aktif penuh.</div>
            </div>
            <div className="surface-muted rounded-[24px] p-4">
